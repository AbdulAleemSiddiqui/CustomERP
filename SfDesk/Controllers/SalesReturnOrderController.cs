...
{
    public class SalesReturnOrderController : Controller
    {
        private SalesReturnOrder old;
        public ActionResult master(int? id)
        {
            //if (id != null)
            //{
            //    SalesReturnOrder p = new SalesReturnOrder() { SO_ID = id.Value, App_Status = "SO_Created" };
            //    p = p.SO_Get_All().Find(x => x.SO_ID == id);
            //    return View(p);
            //}
            return View(new SalesReturnOrder());//{ SO_No = new SalesReturnOrder().SO_Get_New_ID(),Date=DateTime.Now });
        }
        ...
        public ActionResult SO_Approved(int id)
        {
            //if (id != 0)
            //{
            //    SalesReturnOrder p = new SalesReturnOrder() { SO_ID = id, App_Status = "SO_Created" };
            //    p = p.SO_Get_All().Find(x => x.SO_ID == id);
            //    return View(p);
            //}
            return View(new SalesReturnOrder());//{ SO_No = new SalesReturnOrder().SO_Get_New_ID(), Date = DateTime.Now });
        ...
        {
            return View(new SalesReturnOrder());//s { App_Status = "SO_Created" }.SO_Get_All());
        }

        [HttpPost]
        public ActionResult Approve(SalesReturnOrder c)
        {
        ...
        {
            return PartialView("detail", new List<SalesReturnOrder>());
        }
